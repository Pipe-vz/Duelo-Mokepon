...
class Mokepon {
  constructor(nombre, foto, vida, fotoMapa, id = null) {   //con constructor, creamos las propiedades que tendra cada mokepon por defecto
    this.id = id
    this.nombre = nombre
    this.foto = foto
    this.vida = vida
        ...
        res.json() //como los datos vienen en json voy a comenzar a usar res.json. esto es una promesa entoncs debo usar tambein .then
          .then(function ({ enemigos }) { // enemigos es la misma variable que enviams en el index.
            ...
            mokeponesEnemigos = enemigos.map(function (enemigo) {
              ...
              return mokeponEnemigo//creo una variable adicional local para que no se produzca e parpadeo del okepon que recibe de backend
            })
...
function pintarMokeponEnemigo(mokepon) {
  mokepones.forEach((mokepon) => {
    lienzo.drawImage(
      mokepon.mapaFoto,
      mokepon.x = enemigo.x,
